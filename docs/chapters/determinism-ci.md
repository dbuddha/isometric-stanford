...
Geometry-kernel CI covers fixed vectors, zero and excessive dimensions,
misregistered grids, invalid binary and coverage values, depth-response
overflow contracts, reversed thresholds, radius ceilings, absent watershed
markers, stable component discovery, deterministic watershed tie resolution,
and scale-gated line extraction. Guard fixtures require finite-radius Scharr
and composed morphology results to equal their monolithic crops byte for byte.
Whole-supertile connectivity and flood operations are not permitted in the
per-cell path. A committed fixture generated by pinned OpenCV 5.0 and NumPy
2.0.2 supplies a differential oracle without installing either package in
ordinary CI.
